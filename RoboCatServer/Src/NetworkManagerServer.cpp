...
void NetworkManagerServer::ProcessPacket(ClientProxyPtr inClientProxy, InputMemoryBitStream& inInputStream)
{
	...
	int use_counts = inClientProxy.use_count();
	int netID;
	GameObject* yy;
	RoboCatPtr catt;
	...
	case kExitCC:
		...
		catt =  ((Server*)(Engine::sInstance.get()))->GetCatForPlayer(inClientProxy->GetPlayerId());

		yy = (GameObject*)catt.get();

		netID = yy->GetNetworkId();			
		
		if (catt)
		{
			catt->SetDoesWantToDie(true);
		}
		...
		{
			pair.second->GetReplicationManagerServer().ReplicateDestroy(netID);
		}
		...
		{
			if (inClientProxy == it->second)
			{
				mAddressToClientMap.erase(it->first);
				break;
			}

		}		
		
		//((Server*)(Engine::sInstance.get()))->RemoveCatForPlayer(inClientProxy->GetPlayerId());

		ScoreBoardManager::sInstance->RemoveEntry(inClientProxy->GetPlayerId());

		inClientProxy = nullptr;

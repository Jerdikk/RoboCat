...
void NetworkManagerClient::WantToExit()
{
	mState = NCS_Exiting;
	mTimeOfLastExit = 0.f;
}
...
void NetworkManagerClient::UpdateSayingExit()
{
	float time = Timing::sInstance.GetTimef();

	if (time > mTimeOfLastExit + kTimeBetweenHellos)
	{
		SendExitPacket();
		mTimeOfLastExit = time;
	}
...
void NetworkManagerClient::SendExitPacket()
{
	OutputMemoryBitStream exitPacket;

	exitPacket.Write(kExitCC);
	exitPacket.Write(mName);

	SendPacket(exitPacket, mServerAddress);
...
void NetworkManagerClient::HandleByePacket(InputMemoryBitStream & inInputStream)
{
	if (mState == NCS_Exiting)
	{
		LOG("'%s' we are exiting on client as player %d", mName.c_str(), mPlayerId);
		bExit = true;
	}
}
